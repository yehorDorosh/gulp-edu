...
const multipipe =require('multipipe'); // обеденяет несоклько потоков в один для навешивания одно обраотчика событий или ошибки на сразу на все потоки
...
gulp.task('styles', () => {
  return multipipe(
    gulp.src('frontend/styles/main.sass'),
    gulpIf(isDev, sourcemaps.init()),
    sass(),
    gulpIf(isDev, sourcemaps.write('.')), // по умолчанию сорсмап пишется в тот же файл как коментарий, параметр '.' создаст отдельный файл для соср мапа
    gulp.dest('public')
