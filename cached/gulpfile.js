...
gulp.task('styles', () => {
  return gulp.src('frontend/styles/**/*.sass')
    .pipe(cached('styles')) // лучше чем since: потомучто изменения не по дате а по контенту. Решает проблему когда удаленный файл востанавливают через контрл зед. Но since: хорош потому что не читает содержимое файла
...
gulp.task('watch', () => {
  gulp.watch('frontend/styles/**/*.sass', gulp.series('styles')).on('unlink', filepath => {
    remember.forget('styles', path.resolve(filepath)); // застаялет remember очистить кеш по и мени style, для того что бы при удалении файла, не добовлялся в поток отсутсвующий файл. не будет рабоатать т.к. в кешь попадает файл css а событие пытается очистить из кеша sass. Нужно заменить в filepath расширение файла на css
    delete cached.caches.styles[path.resolve(filepath)]; // очистить кешь кешеда
  });
